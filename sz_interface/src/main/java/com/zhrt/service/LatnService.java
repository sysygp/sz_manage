...
public class LatnService {
	...
	@Transactional
	public Latn getByProName(String proName){
		Latn latn = null;
		try {
			Map<String,Object> paramMap = new HashMap<String,Object>();
			paramMap.put("latnLevel", "2");
			paramMap.put("latnName", proName);
			latn = latnDao.get(paramMap);
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Transactional
	public Integer getProidByProName(String proName){
		try {
			Latn latn = getByProName(proName);
			if(latn != null){
				return latn.getLatnId();
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
